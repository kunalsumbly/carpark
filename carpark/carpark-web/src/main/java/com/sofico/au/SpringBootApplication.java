...
@Configuration // is the used to register bean and enable annotations is same as defining a bean in xml and using <context:annotation-config/>
...
@ComponentScan({"com.au.sofico"})// seems same as <context:component-scan> which not only enables the annotations but also registers the bean in the context or IOC container
